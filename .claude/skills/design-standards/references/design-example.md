...
<!-- ✅ Cada REQ tem ao menos um item -->
<!-- ✅ Cada NFR tem ao menos um item -->
<!-- ✅ Todos os componentes têm item de implementação -->
<!-- ✅ Cada Scenario do .feature tem item de teste -->
<!-- ✅ Itens verificáveis (sim/não) sem vaguidão -->

### Fase 1: Modelo
- [ ] Criar entidade `VerificationCode` com campos: id, driver_id, code, channel, expires_at, used_at, created_at — REQ-1, REQ-9, REQ-10
- [ ] Criar migration: tabela `verification_codes` com FK para `delivery_drivers`
- [ ] Definir schema Redis para `RateLimitEntry` com prefixos `rate_limit:contact:` e `rate_limit:ip:` — NFR-4

### Fase 2: Domínio
- [ ] Implementar `PasswordRecoveryDomain.generateCode()` — gera OTP numérico de 6 dígitos — REQ-1
- [ ] Implementar `PasswordRecoveryDomain.validateCode(code, entry)` — verifica expiração e uso único — REQ-9, REQ-10, NFR-2
- [ ] Implementar `PasswordRecoveryDomain.validatePasswordStrength(password)` — 8+ chars, maiúsculas, minúsculas, números, especiais — REQ-8
- [ ] Implementar `PasswordRecoveryDomain.validatePasswordMatch(password, confirmation)` — REQ-7

### Fase 3: Infraestrutura
- [ ] Implementar `DeliveryDriverRepository.findByContact(channel, contact)` — REQ-1, REQ-6
- [ ] Implementar `DeliveryDriverRepository.updatePasswordHash(driverId, hash)` — REQ-3
- [ ] Implementar `VerificationCodeRepository.save(driverId, code, channel, expiresAt)` — REQ-1
- [ ] Implementar `VerificationCodeRepository.findValid(contact, code)` — REQ-2, REQ-9
- [ ] Implementar `VerificationCodeRepository.markAsUsed(codeId)` — REQ-10
- [ ] Implementar `RateLimiterAdapter.check(contact, ip)` — retorna bloqueado/livre — NFR-4, REQ-11
- [ ] Implementar `RateLimiterAdapter.increment(contact, ip)` — incrementa contador — NFR-4
- [ ] Implementar `RateLimiterAdapter.reset(contact)` — zera contador após sucesso — NFR-4
- [ ] Implementar `EmailNotificationAdapter.send(email, code)` — REQ-1, NFR-1
- [ ] Implementar `SmsNotificationAdapter.send(phone, code)` — REQ-1, NFR-1
- [ ] Implementar `PasswordService.hash(password)` com bcrypt custo ≥ 100ms — NFR-5
- [ ] Configurar logging de todas as tentativas de solicitação (sucesso, falha, bloqueio) com retenção de 1 ano — NFR-6

### Fase 4: API
- [ ] Implementar `POST /api/v1/password-recovery/request` com resposta neutra para contato não cadastrado — REQ-1, REQ-5, REQ-6, NFR-3
- [ ] Implementar `POST /api/v1/password-recovery/verify` com validação de OTP — REQ-2, REQ-9
- [ ] Implementar `POST /api/v1/password-recovery/reset` com validação de senha — REQ-3, REQ-4, REQ-7, REQ-8
- [ ] Implementar handler `429` para bloqueio por rate limit em `/request` — REQ-11, NFR-4
- [ ] Implementar emissão de `reset_token` JWT (15 min) no `/verify` — DT-4

### Fase 5: Testes
- [ ] Teste E2E: Scenario "Recuperação de senha com email e código válido"
- [ ] Teste E2E: Scenario "Solicitação de código com dado de formato inválido"
- [ ] Teste E2E: Scenario "Solicitação de código com dado não cadastrado" — verificar resposta idêntica ao sucesso (NFR-3)
- [ ] Teste E2E: Scenario "Informar código de verificação expirado ou inválido"
- [ ] Teste E2E: Scenario "Redefinição de senha com campos de confirmação divergentes"
- [ ] Teste E2E: Scenario "Redefinição de senha sem atender aos requisitos mínimos"
- [ ] Teste E2E: Scenario "Bloqueio por tentativas excessivas de solicitação de código"
- [ ] Teste E2E: Scenario "Tentativa de reutilizar código após uso bem-sucedido"
- [ ] Teste unitário: `PasswordRecoveryDomain.validateCode` — casos: expirado, já usado, válido
- [ ] Teste unitário: `PasswordRecoveryDomain.validatePasswordStrength` — casos: cada requisito mínimo isolado
- [ ] Teste de performance: tempo de hash bcrypt ≥ 100ms — NFR-5
- [ ] Teste de carga: entrega do código em até 30s para p95 — NFR-1
