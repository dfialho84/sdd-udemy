...
6. **Anuncie o início** da sessão:
    ...
    Vamos construir as 7 seções juntos. Começando pela Visão Geral Técnica.
    ```
...
4. Verifique se todos os cenários BDD de `scenarios.feature` têm correspondência no Fluxo de Execução ou no Checklist de Testes
5. Se algum requisito ou cenário não tiver correspondência, adicione um item na seção Checklist de Implementação ou uma nota em Decisões Técnicas
...
Arquivo: docs/features/<slug>/design.md
Seções: 7 | Requisitos funcionais cobertos: <N>/<total> | NFRs cobertos: <N>/<total> | Cenários BDD cobertos: <N>/<total>

Próximos passos sugeridos:
- Revisar o Checklist de Implementação com o time de desenvolvimento
- Usar o design.md como contexto ao criar tasks de implementação
```
...
- Cada item deve ser verificável (sim/não), específico ao componente ou requisito
- Agrupe por fase: (1) Modelo, (2) Domínio, (3) Infraestrutura, (4) API, (5) Testes
- Inclua ao menos um item por requisito funcional, um item por NFR e um item de teste por cenário BDD
