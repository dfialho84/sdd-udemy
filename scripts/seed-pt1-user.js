#!/usr/bin/env node
/**
 * Seed de usuario para PT-1 — Latencia de autenticacao bem-sucedida (k6)
 * Rastreabilidade: T-61 · NFR-1
 *
 * Cria (ou recria) um usuario active com credenciais fixas usadas pelo
 * script k6/authenticate-user.load-test.js.
 *
 * Credenciais inseridas:
 *   username  : pt1-loadtest
 *   email     : pt1-loadtest@example.com
...
// Credenciais fixas do usuario de carga PT-1
const PT1_USER = {
  id: "pt1-loadtest-user-id-fixed-uuid",
  name: "PT1 LoadTest User",
  username: "pt1-loadtest",
  email: "pt1-loadtest@example.com",
  password: "Senha@123Forte",
...
async function seedPt1User() {
  const connection = await mysql.createConnection(DATABASE_URL);

  try {
    console.log("Gerando hash argon2id (pode levar alguns segundos)...");

    const passwordHash = await argon2.hash(PT1_USER.password, {
    ...
    await connection.execute(
      ...
      [PT1_USER.username, PT1_USER.email],
    );
    await connection.execute(
    ...
    await connection.execute(
      ...
      [PT1_USER.id, PT1_USER.username, PT1_USER.email],
    );
    ...
    console.log("");
    console.log("Usuario PT-1 criado com sucesso:");
    console.log(`  username : ${PT1_USER.username}`);
    console.log(`  email    : ${PT1_USER.email}`);
    console.log(`  password : ${PT1_USER.password}`);
    console.log(`  status   : active`);
    console.log("");
    console.log("Agora execute:");
    console.log("  npm run dev");
    console.log("  k6 run k6/authenticate-user.load-test.js");
  } finally {
    await connection.end();
  }
}

seedPt1User().catch((err) => {
