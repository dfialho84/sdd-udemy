...
| avatar_url    | VARCHAR (nullable)        | Caminho relativo ao servidor do arquivo de avatar salvo (`/uploads/avatars/<uuid>.<ext>`); null se não fornecido (REQ-1) |
...
2. `RegisterUserHandler` extrai campos e arquivo do `FormData`. Valida campos obrigatórios, formato de email, política de senha e coincidência das senhas. Se qualquer validação falhar, retorna HTTP 400 com a mensagem de erro correspondente e nenhum registro é criado (REQ-2, REQ-4, REQ-5, REQ-6, REQ-7).
