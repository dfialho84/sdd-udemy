...
**Concluída quando:** O Scenario GH-1 passa no Cypress; a URL muda para `/register` após o clique no link; os seis campos do formulário estão visíveis no DOM.
...
## REQ-2 — Exibir formulário de cadastro

> When the visitor accesses the registration page, the system shall display a form containing the following fields: full name, email address, password, password confirmation, date of birth, and profile photo (optional).
...
**Rastreabilidade:** REQ-2 · REQ-6 · REQ-7 · REQ-8 · REQ-9 · REQ-10
**Depende de:** —
...
### T-76: Implementar `RegisterPage` — formulário de cadastro em `/register`

- [x] Implementar a página React em `src/app/register/page.tsx` que exibe o formulário de cadastro com os campos nome, email, senha, confirmação de senha, data de nascimento e foto de perfil. Gerenciar o estado do formulário com react-hook-form e validação Zod no lado do cliente. Submeter os dados ao `POST /api/auth/register` via `multipart/form-data`. Ao receber resposta de sucesso (HTTP 200), exibir a tela de confirmação de envio de email (REQ-5). A página deve estar em conformidade com WCAG 2.1 AA (NFR-9).

**Rastreabilidade:** REQ-2 · REQ-5 · NFR-9 · DT-8
**Depende de:** —
**Concluída quando:** A página `/register` renderiza o formulário com os seis campos; o formulário é submetido como `multipart/form-data`; após HTTP 200 do endpoint, a tela de confirmação de envio é exibida; a página compila sem erros e não importa dependências de domínio.
...
**Depende de:** T-76
**Concluída quando:** Os quatro casos de UT-10 passam; nenhuma violação axe é reportada para a `RegisterPage`; todos os campos têm labels associados no DOM.
...
**Concluída quando:** `POST /api/auth/register` com dados válidos retorna HTTP 200 com a mensagem correta; o usuário é persistido com status `pending` no banco e `avatar_key` preenchido com a object key do MinIO; arquivo com tipo MIME inválido ou acima de 2 MB retorna HTTP 400 sem criar nenhum registro nem enviar nenhum objeto ao MinIO.

---

### T-21: Cobrir IT-6 — `RegisterUserHandler POST /api/auth/register` (integração)

- [x] Implementar o teste de integração IT-6 cobrindo todos os casos: dados válidos sem avatar (HTTP 200, usuário `pending` com `avatar_key = null` no banco), dados válidos com avatar JPEG válido até 2 MB (HTTP 200, `avatar_key` preenchido com object key no formato `avatars/<uuid>.jpg`, objeto gravado no bucket MinIO de teste), avatar com tipo MIME não permitido como `image/gif` (HTTP 400, nenhum registro criado, nenhum objeto gravado no MinIO), avatar acima de 2 MB (HTTP 400, nenhum registro criado, nenhum objeto gravado no MinIO), campo obrigatório ausente (HTTP 400), email inválido (HTTP 400), senha fora da política (HTTP 400), senhas divergentes (HTTP 400), email duplicado (HTTP 409) e quarta tentativa do mesmo IP em 15 min (HTTP 429). Remover objetos de avatar gravados no MinIO após os testes.

**Rastreabilidade:** REQ-2 · REQ-3 · REQ-4 · REQ-5 · REQ-6 · REQ-7 · REQ-8 · REQ-9 · REQ-10 · REQ-11 · NFR-6
**Depende de:** T-20
**Concluída quando:** Todos os casos de IT-6 passam contra banco MySQL de teste, Mailhog disponível e MinIO disponível com bucket de teste criado; nenhum objeto de avatar permanece no bucket após os testes.

---

### T-22: Cobrir GH-2 — Scenario "Cadastro realizado com dados validos" (E2E)

- [x] Implementar os step definitions e o teste E2E Gherkin GH-2 cobrindo: navegação para `/register`, preenchimento de todos os campos válidos, submissão do formulário, verificação da mensagem de link enviado e verificação da presença do email com link de confirmação na API do Mailhog.

**Rastreabilidade:** REQ-3 · REQ-4 · REQ-5 · NFR-3 · Scenario: "Cadastro realizado com dados validos"
**Depende de:** T-20
**Concluída quando:** O Scenario GH-2 passa no Cypress; a mensagem de confirmação é exibida e o email aparece no Mailhog.

---

### T-23: Cobrir PT-2 — latência de `POST /api/auth/register` sob carga (performance)

- [x] Implementar o teste de carga PT-2 com k6: 10 usuários virtuais simultâneos por 60 segundos enviando dados válidos para `POST /api/auth/register`. O teste deve falhar se o p95 de latência exceder 3.000 ms.
...
**Rastreabilidade:** REQ-4 · REQ-12 · REQ-14 · REQ-15 · REQ-17 · NFR-5
**Depende de:** T-01
...
**Rastreabilidade:** REQ-4 · REQ-14 · REQ-17 · NFR-5
**Depende de:** T-24
...
## REQ-6 — Bloquear envio com campo obrigatório em branco

> If any required field (full name, email address, password, password confirmation, or date of birth) is blank at form submission, the system shall block the submission and display an error message indicating which field is missing.
...
## REQ-7 — Rejeitar email já cadastrado
...
**Rastreabilidade:** REQ-7 · REQ-3 · REQ-12 · REQ-15
**Depende de:** T-01
...
- [x] Implementar o `RegisterUserUseCase` com o método `execute`. Incluir a verificação de unicidade de email via `UserRepository.findByEmail`: se o email já existir, lançar erro com código 409 e a mensagem exigida por REQ-7, sem criar nenhum registro. O caso de uso deve orquestrar todo o fluxo feliz descrito no design (hash de senha, criação do usuário, geração e persistência do token, envio de email e log estruturado).

**Rastreabilidade:** REQ-7 · REQ-3 · REQ-4 · REQ-11 · NFR-10
**Depende de:** T-05
...
**Rastreabilidade:** REQ-7 · REQ-3 · REQ-4 · REQ-11 · NFR-4 · NFR-10 · Scenario: "Cadastro realizado com dados validos"
**Depende de:** T-06
...
## REQ-8 — Rejeitar senha fora da política de segurança
...
## REQ-9 — Rejeitar confirmação de senha divergente
...
## REQ-10 — Rejeitar email com formato inválido
...
## REQ-11 — Não criar registro em caso de validação inválida
...
- [x] Implementar os step definitions e o teste E2E Gherkin GH-3 cobrindo todas as situações do `Scenario Outline`: email duplicado, senha inválida, senhas divergentes, nome em branco, data de nascimento em branco e email inválido. Para cada situação, verificar a mensagem de erro exibida e a ausência de novo registro no banco.

**Rastreabilidade:** REQ-6 · REQ-7 · REQ-8 · REQ-9 · REQ-10 · REQ-11 · Scenario: "Cadastro com dados invalidos no formulario"
**Depende de:** T-04 · T-07 · T-11 · T-17 · T-18
**Concluída quando:** Todos os exemplos do Scenario Outline GH-3 passam no Cypress; o banco de teste não contém novos registros após cada caso inválido.

---

## REQ-12 — Ativar conta via link de confirmação válido
...
- [x] Implementar o `ConfirmAccountUseCase` com o método `execute(token: string)`. O caso de uso deve: (1) buscar o token via `ConfirmationTokenRepository.findByToken` — retornar erro 404 se não encontrado; (2) verificar `isUsed()` — retornar sinal de already_confirmed e logar se já utilizado (NFR-11); (3) verificar `isExpired()` — remover cadastro pendente via `UserRepository.delete`, logar e retornar sinal de expirado se expirado (REQ-15); (4) marcar token como usado via `markAsUsed`; (5) ativar conta via `UserRepository.activate`; (6) logar sucesso (NFR-11).

**Rastreabilidade:** REQ-12 · REQ-13 · REQ-14 · REQ-15 · REQ-16 · REQ-17 · REQ-18 · NFR-5 · NFR-11
...
**Rastreabilidade:** REQ-12 · REQ-14 · REQ-17 · NFR-5
**Depende de:** T-31
...
**Rastreabilidade:** REQ-12 · REQ-13 · REQ-14 · REQ-15 · REQ-16 · REQ-17 · REQ-18 · NFR-5 · NFR-11 · Scenario: "Confirmacao de conta via link valido"
**Depende de:** T-36
...
**Rastreabilidade:** REQ-12 · REQ-13 · REQ-14 · Scenario: "Confirmacao de conta via link valido"
**Depende de:** T-39
...
## REQ-13 — Exibir mensagem de sucesso e link de acesso após confirmação
...
**Rastreabilidade:** REQ-13 · REQ-16 · REQ-17
**Depende de:** T-39
...
## REQ-14 — Invalidar o token imediatamente após o primeiro uso bem-sucedido
...
_(REQ-14 é coberto pelas tasks T-38 que implementa o método `markAsUsed` no repositório, T-36 que orquestra a invalidação imediata no caso de uso, e T-48 que verifica a prevenção de reuso como teste de segurança ST-2. A rastreabilidade está declarada nessas tasks.)_

---

## REQ-15 — Rejeitar link expirado e remover cadastro pendente
...
**Rastreabilidade:** REQ-15 · REQ-16 · Scenario: "Confirmacao de cadastro com link expirado"
**Depende de:** T-24
...
## REQ-16 — Exibir mensagem de link expirado e opção de novo cadastro
...
_(REQ-16 é coberto pelas tasks T-39, T-43, T-44, T-45 e T-71 que implementam e testam o retorno HTTP 302 com `error=expired`, a mensagem de expiração e o link para `/register` na página `/confirm`.)_

---

## REQ-17 — Rejeitar link já utilizado
...
**Rastreabilidade:** REQ-17 · REQ-18 · Scenario: "Confirmacao de cadastro com link ja utilizado"
**Depende de:** T-24
...
**Rastreabilidade:** NFR-5 · REQ-14 · REQ-17 · REQ-18
**Depende de:** T-39
**Concluída quando:** Os dois casos de ST-2 passam; o segundo uso não altera nenhum estado no banco.

---

## REQ-18 — Não alterar status da conta com link já utilizado

> If a confirmation link has already been used, the system shall not alter the status of the already active account.

_(REQ-18 é coberto pelas mesmas tasks de REQ-17: T-36, T-38, T-40, T-43, T-46, T-47 e T-48 que implementam e verificam a invariante de que o status da conta não é alterado em caso de reuso de token.)_

---

## NFRs sem REQ direto

### NFR-6 — Rate limiting de tentativas por IP
...
### NFR-10 — Log estruturado JSON para tentativas de cadastro e falhas de email
...
**Rastreabilidade:** NFR-10 · REQ-3 · REQ-4
**Depende de:** T-06
...
### NFR-11 — Log estruturado JSON para eventos de confirmação de conta
...
**Rastreabilidade:** NFR-11 · REQ-12 · REQ-15 · REQ-17
**Depende de:** T-36
...
- [x] Implementar a verificação de logs como parte dos testes IT-4 e IT-6: confirmar que logs JSON são emitidos nos eventos de criação de cadastro, falha de email e confirmação de conta (bem-sucedida, expirada e já utilizada), com os campos exigidos por NFR-10 e NFR-11.

**Rastreabilidade:** NFR-10 · NFR-11
...
### NFR-8 — Disponibilidade 99,9% ao mês
...
### NFR-12 — Logs entregues ao Loki e consultáveis por requestId
...
**Rastreabilidade:** NFR-12
**Depende de:** T-56
**Concluída quando:** Após subir o ambiente com `docker compose up`, um `logger.info(...)` é recuperável no Grafana Explore com o filtro `{job="kanban-app"}`; logs de NFR-10 e NFR-11 aparecem com `requestId` e `tipoEvento` nos campos.

---

### NFR-7 — Suporte a 100 usuários simultâneos
...
### NFR-13 — Acesso protegido à foto de perfil: autenticação obrigatória e rejeição por status de conta
...
**Depende de:** T-74
**Concluída quando:** Todos os seis casos de IT-8 passam com banco MySQL de teste e MinIO reais; os logs JSON contêm os campos obrigatórios do NFR-13; nenhuma presigned URL é gerada nos casos de rejeição 401/403.
